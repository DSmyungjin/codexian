...
Two channels, ordered by reliability:
...
2. **`session-start.mjs` hook** (only on Codex CLI versions where SessionStart hooks fire). The script is generated by `codexian spec init` and emits the spec context as a JSON envelope per Codex's hook protocol.

### Codex CLI version recommendation

The SessionStart hook channel is currently *broken on Codex 0.129.0 and later* — upstream regression tracked at [openai/codex#21639](https://github.com/openai/codex/issues/21639), still open as of this writing. Verified empirically: hooks fire on 0.128.0, do not fire on 0.130.0 or 0.131.0-alpha.4.

```bash
# Recommended pin while the regression is unresolved
npm install -g @openai/codex@0.128.0
```

`codexian spec doctor` checks the local `codex --version` and emits a WARN when it detects an affected build, so you are never silently downgraded to AGENTS.md-only operation without noticing.

If you stay on the latest Codex CLI, the AGENTS.md mandatory directive is the sole load channel — operational and verified in real `codex exec` runs, but less robust under long sessions than hooks would be.
